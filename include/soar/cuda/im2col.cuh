...
/**
 * @brief CUDA im2col kernel mirroring PyTorch at::native::im2col_kernel.
 */
...
inline void im2col(
    ...
    int num_blocks = GET_BLOCKS(num_kernels, 512);
    im2col_kernel<<<num_blocks, 512, 0, stream>>>(
        num_kernels,
        data_im,
        height,
        width,
...
/**
 * @brief CUDA col2im device and kernel mirroring PyTorch at::native::col2im_kernel.
 */
...
__forceinline__ __device__ void col2im_device(
    ...
    const int64_t w_col_end = min((int64_t)(w_im / stride_width + 1), width_col);
    const int64_t h_col_start = (h_im < kernel_extent_h)
        ? 0
        : (h_im - kernel_extent_h) / stride_height + 1;
    const int64_t h_col_end = min((int64_t)(h_im / stride_height + 1), height_col);
        ...
        for (int64_t w_col = w_col_start; w_col < w_col_end; w_col += 1) {
            ...
            if (h_k % dilation_height == 0 && w_k % dilation_width == 0) {
                ...
                int64_t data_col_index =
                    (((c_im * kernel_h + h_k) * kernel_w + w_k) * height_col + h_col) * width_col + w_col;
...
inline void col2im(
    ...
    if (num_kernels == 0) return;
    int num_blocks = GET_BLOCKS(num_kernels, 512);
    col2im_kernel<dt, accT><<<num_blocks, 512, 0, stream>>>(
        num_kernels,
        data_col,
        height,
        width,
